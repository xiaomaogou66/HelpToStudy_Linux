...
| 命令 | 作用 |
|---|---|
| `./同步日历.sh --dry-run` | **只看**会推送什么，不联网、不动日历 |
| `./同步日历.sh` | 真正同步（周日写完周决策后跑这一条） |
| `./同步日历.sh --check` | 检查凭据和日历是否可用 |
| `./同步日历.sh --completed drop` | 已完成的任务从日历移除（默认加 ✅ 前缀保留） |

也可以直接双击同目录下的 **`同步日历.desktop`**。
...
| 想干嘛 | 怎么写 |
|---|---|
| 换端口 / 换机器 | `SYNC_PROXY=http://127.0.0.1:7890 ./同步日历.sh` |
...
| 现象 | 解决 |
|---|---|
| **`❌ 直连和所有代理候选都连不上 Google API`** ／ 终端 / 双击 `.desktop` 时超时 | 代理客户端没开。脚本固定走 `127.0.0.1:7897` → 打开 Clash Verge，确认它在听（`ss -ltn \| grep 7897`）；端口不是 7897 就 `SYNC_PROXY=http://127.0.0.1:7900 ./同步日历.sh` |
| **`SSLEOFError` / `TimeoutError` / 打印「⏳ 网络抖动，等 Ns 重试」** | 本机代理（7897）节点抖动，**脚本会自动退避重试最多 6 次**。若最终仍失败：换节点/重启代理再跑。实测同一分钟内 3 次里约 1 次通，重试基本都能过 |
| **请求卡 20 秒后 `TimeoutError`** | 走了 http 代理但 venv 里没 PySocks（httplib2 会静默直连）→ `../_工具/.venv/bin/python -m pip install -r requirements.txt` |
