...
用法：
    ./打卡.sh              # 生成本周（或补齐缺失项，不动已有勾选）
    ./打卡.sh 2026-W39     # 指定周
    ./打卡.sh --print      # 只打印不上磁盘

写进哪：`05-日程安排/06-日志/<周号>/打卡.md`（周文件夹由 `周文件夹.sh` 建，
...
LOG_DIR = VAULT / "05-日程安排/06-日志"   # 里面一周一个 <周号>/ 文件夹（见 周文件夹.sh）
...
def render(week: str, days: list[tuple[datetime, list[str]]], existing: dict,
    ...
    for d, items in days:
        ...
        lines.append("")
    lines.append("> 打卡页由 `_脚本/打卡.sh` 生成；重复运行只补缺失项，**不会清掉你已勾的**。")
    lines.append("")
    return "\n".join(lines)
