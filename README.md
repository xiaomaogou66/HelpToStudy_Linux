...
| `05-日程安排/_脚本/同步日程.sh` | 日程同步：`ics`（本机文件）与 `gcal`（Google 日历）两个地位同等的后端 |
| `05-日程安排/_脚本/自检.sh` | 烟测：配置/打卡/日记/两个后端/令牌 一次全检（不联网、不改数据） |
...
- **章节识别**支持中/英/意/西/法常见标题（含意大利语序数词课名、目录页码格式）；
  章标题被 OCR 整批打掉时（如《某外语教材》每课用了装饰字体的 UNIDAD 标题，
...
│   ├── _脚本/           ← 引擎：同步日程.sh（ics/gcal 双后端）· 导出ics.py · sync_gcal.py · 打卡.sh · 周文件夹.sh · today.sh · 自检.sh
